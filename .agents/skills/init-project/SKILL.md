...
- **docs/ 产品层重置**：按 `docs/README.md`「新项目接手指南」处理——`docs/product/vision.md`、`prd.md` 重写为新项目内容，`backlog.md`、`ROADMAP.md` 清空重建；`dev/`、`ops/`、`adr/`、`task/` 原样保留。
- **技能分发目录**：`.agents/skills/`、`.opencode/skills/`、`.zcode/skills/` 由 `scripts/sync-agents.sh` 从 `.claude/skills/` 镜像维护，替换脚本会一并处理，无需手动干预；若替换后内容异常，重跑一次分发脚本即可。
